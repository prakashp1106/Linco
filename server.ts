...
app.get("/api/maps/autosuggest", async (req, res) => {
  try {
    ...
    // 1. Try MapmyIndia first (if API key is configured)
    if (apiKey) {
      try {
    ...
    // 2. Fall back to OpenStreetMap Nominatim if MapmyIndia failed, returned no results, or API key missing
    if (!mapplsSuccess) {
      try {
...
app.get("/api/maps/revgeocode", async (req, res) => {
  try {
    ...
    // 2. Fall back to OpenStreetMap Nominatim if MapmyIndia failed, returned no address, or API key missing
    if (!mapplsSuccess) {
      try {
