...
export const TimelineSection: React.FC<TimelineSectionProps> = ({
  ...
  return (
    <div
      className="p-4 sm:p-5 rounded-2xl bg-white border border-slate-200 space-y-4 text-left shadow-2xs"
      id="timeline-tracer"
    >
      {/* Header */}
      ...
      <div className="space-y-2.5">
        ...
        <button
          ...
          className="w-full py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white text-xs font-semibold flex items-center justify-center gap-2 transition cursor-pointer disabled:opacity-50 shadow-2xs active:scale-95"
        >
            ...
            <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs">
              <div>
                ...
                {timelineData.reasoning && (
                  <p className="text-[11px] text-slate-600 mt-1 italic font-sans">
                    "{timelineData.reasoning}"
                  </p>
                )}
              </div>
            ...
            <div className="space-y-2">
              {timelineData.events?.map((evt, idx) => {
                ...
                return (
                  <div
                    key={evt.id || idx}
                    className="p-3 rounded-xl bg-white border border-slate-200 flex items-start justify-between gap-3 group hover:border-slate-300 transition shadow-2xs"
                  >
                    <div className="flex items-start gap-2.5">
                      <div className="w-6 h-6 rounded-full bg-slate-50 border border-slate-200 text-slate-600 text-[10px] font-bold flex items-center justify-center shrink-0 mt-0.5 shadow-2xs">
                        {idx + 1}
                      </div>
                      <div>
              ...
              <div className="p-3 rounded-xl bg-white border border-slate-200 space-y-2 shadow-2xs">
                ...
                <div className="flex justify-end gap-2 pt-1">
                  <button
                    ...
                    className="px-2.5 py-1 rounded bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 text-xs font-semibold cursor-pointer shadow-2xs"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="px-3 py-1 rounded bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold cursor-pointer shadow-2xs active:scale-95"
                  >
                    Save Checkpoint
                  </button>
                </div>
