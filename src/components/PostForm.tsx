...
export const PostForm: React.FC<PostFormProps> = ({ onSubmit }) => {
  ...
  return (
    <div className="bg-slate-950/40 border border-slate-900 rounded-3xl p-6 md:p-10 shadow-2xl backdrop-blur-xl relative" id="post-form-card">
        ...
        <div className="mb-8 p-5 md:p-6 rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-950/10 via-slate-950/40 to-cyan-950/10 relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="flex items-center justify-center gap-2 px-4 py-3 rounded-xl bg-violet-600/10 hover:bg-violet-600/20 border border-violet-500/20 text-xs md:text-sm font-extrabold text-violet-300 hover:text-violet-100 transition duration-200 cursor-pointer text-center">
              <Camera size={16} className={ai.photoLoading ? "animate-spin" : ""} />
              {ai.photoLoading ? "Reading photo..." : "Analyze Photo"}
              <input
                type="file"
                accept="image/*"
            ...
            <button
              ...
              className={`flex items-center justify-center gap-2 px-4 py-3 rounded-xl border text-xs md:text-sm font-extrabold transition duration-200 text-center cursor-pointer ${
                ai.voiceActive
                  ? "bg-rose-500/20 border-rose-500 text-rose-300 animate-pulse"
                  : "bg-rose-600/10 hover:bg-rose-600/20 border-rose-500/20 text-rose-300 hover:text-rose-100"
              }`}
            >
              {ai.voiceActive ? <Mic size={16} className="text-rose-400 animate-ping" /> : <Mic size={16} className={ai.voiceLoading ? "animate-spin" : ""} />}
              {ai.voiceLoading ? "Deducing..." : ai.voiceActive ? "Stop Speaking" : "Speak Voice"}
            </button>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-center gap-6 mb-8 text-xs font-mono font-bold tracking-widest text-slate-500 uppercase">
        <span className={form.step === 1 ? "text-cyan-400 border-b border-cyan-400 pb-1" : ""}>Step 1</span>
        <ChevronRight size={12} />
        <span className={form.step === 2 ? "text-cyan-400 border-b border-cyan-400 pb-1" : ""}>Step 2</span>
        <ChevronRight size={12} />
        <span className={form.step === 3 ? "text-cyan-400 border-b border-cyan-400 pb-1" : ""}>Step 3</span>
      </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-extrabold text-emerald-400 hover:text-emerald-300 uppercase transition duration-150 disabled:opacity-50 cursor-pointer"
                >
                  <Sparkles size={14} className={ai.enhanceLoading ? "animate-spin" : ""} />
                  Polish with AI
                </button>
              </div>
              <textarea
        ...
        {form.step === 3 && (
          ...
          >
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-extrabold text-slate-200 uppercase tracking-wider pb-2 border-b border-slate-900">
                AI Assistants &amp; Deducers
              </h3>
              ...
              {form.fType === "Lost" ? (
                ...
                <p className="text-xs text-slate-500">
                  Timeline tracing is optimized for Lost reports. Good luck!
                </p>
              )}
