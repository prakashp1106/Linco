...
def translate_chunk(texts, target):
...
def translate_language(target_code, api_code, chunk_size=25):
        ...
        try:
            with open(cache_file) as f:
                ...
                if len(cached) == len(missing_keys):
                    print(f"[{target_code}] Already cached ({len(cached)} keys). Skipping.")
                    return cached
        except Exception:
            pass
    ...
    for i in range(0, len(missing_keys), chunk_size):
        ...
        for attempt in range(3):
            try:
                trans_v = translate_chunk(chunk_v, api_code)
        ...
        if not success:
            print(f"[{target_code}] Chunk {i} falling back to single...")
            for k, v in zip(chunk_k, chunk_v):
                try:
                    res[k] = translate_chunk([v], api_code)[0]
                except Exception:
                    res[k] = v
                time.sleep(0.1)
        time.sleep(0.2)
    ...
    print(f"[{target_code}] Done! Cached {len(res)} keys.")
    return res
...
for code, api_code, export_name, display_name in INTL_CONFIGS:
    translate_language(code, api_code)
...
print("ALL 16 INTERNATIONAL LANGUAGES WRITTEN SUCCESSFULLY!")
